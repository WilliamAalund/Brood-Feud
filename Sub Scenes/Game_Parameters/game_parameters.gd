...
# --- SINGLETONS --- 
var gameOccurring = false #Value managed by Main level node. When this value is true, certain game logic is permitted to run.
...
const BIRD_ATTACKED_DAMAGE_CHUNK = 3 # Flat value that is decremented form a bird's satiation if they are attacked.
...
const PLAYER_BIRD_BLEED_RATE = 0.02 # Rate at which the player bird bleeds
const PLAYER_BIRD_ATTACKED_DAMAGE_CHUNK = 7 # Flat value that is decremented from the player's satiation if they are attacked.
...
const STARVATION_THRESHOLD = 30 # Constant used to determine if starving_color_block is visible or not
const GROW_AGE_ONE = 4 # Constant used to determine when the body of the player bird will grow older. Default age is 4
const GROW_AGE_TWO = 7 # Constant used to determine when the head of the player bird will grow older. Default age is 7
...
const MAXIMUM_AGGRO_VALUE = 80 # Maximum amount of aggro that an AI bird can have
const AI_BIRD_INTERACT_INPUT_DELAY = 0.2 # In seconds
const AI_BIRD_INTERACT_LENGTH = 0.2 # In seconds
const AI_BIRD_INTERACT_COOLDOWN = 1.1 # In seconds
...
const TIME_TO_STAY_AT_NEST = 2.5 # The time that the predator will stay inside of the nest before leaving
